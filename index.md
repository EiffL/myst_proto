...
{astra:value}`outputs/bao_alpha_values tracer=elg1 recon=Pre col=alpha1_std` to
{astra:value}`outputs/bao_alpha_values tracer=elg1 recon=Post col=alpha1_std`. The
strongest single constraint, the combined LRG3+ELG1 bin, reaches
$\alpha_\mathrm{iso} =$
{astra:value}`outputs/bao_alpha_values tracer=lrg3_elg1 recon=Post col=alpha1_mean err=alpha1_std`.
Propagated to distances, we obtain $D_M/r_d$, $D_H/r_d$, and $D_V/r_d$ at the
percent level per tracer — for LRG3+ELG1, $D_V/r_d =$
{astra:value}`outputs/bao_distance_table tracer=lrg3_elg1 col=DV_over_rd pm` at
$z_\mathrm{eff} =$ {astra:value}`outputs/bao_distance_table tracer=lrg3_elg1 col=z_eff` —
...
{astra:value}`outputs/bao_alpha_values tracer=lrg3 recon=Post col=alpha1_mean err=alpha1_std`
...
$\chi^2 =$ {astra:value}`outputs/bao_alpha_values tracer=lrg3_elg1 recon=Post col=chi2`
for {astra:value}`outputs/bao_alpha_values tracer=lrg3_elg1 recon=Post col=dof` degrees
of freedom; QSO is the least well-behaved, at $\chi^2 =$
{astra:value}`outputs/bao_alpha_values tracer=qso recon=Post col=chi2` for
{astra:value}`outputs/bao_alpha_values tracer=qso recon=Post col=dof`.
...
$D_M/r_d =$
{astra:value}`outputs/bao_distance_table tracer=lrg3_elg1 col=DM_over_rd pm`,
$D_H/r_d =$ {astra:value}`outputs/bao_distance_table tracer=lrg3_elg1 col=DH_over_rd pm`,
and $D_V/r_d =$
{astra:value}`outputs/bao_distance_table tracer=lrg3_elg1 col=DV_over_rd pm`. The 1D
tracers contribute $D_V/r_d$ alone:
{astra:value}`outputs/bao_distance_table tracer=bgs col=DV_over_rd pm` at
$z_\mathrm{eff} =$ {astra:value}`outputs/bao_distance_table tracer=bgs col=z_eff` (BGS),
{astra:value}`outputs/bao_distance_table tracer=elg1 col=DV_over_rd pm` at
{astra:value}`outputs/bao_distance_table tracer=elg1 col=z_eff` (ELG1), and
{astra:value}`outputs/bao_distance_table tracer=qso col=DV_over_rd pm` at
{astra:value}`outputs/bao_distance_table tracer=qso col=z_eff` (QSO).
